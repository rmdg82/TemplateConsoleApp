...
namespace TemplateConsoleApp.Helpers
{
    /// <summary>
    ...
    public static class PrettyConsole
    {
        ...
        private static readonly Lazy<Regex> _colorBlockRegEx = new(
        ...
        public static void WriteEmbeddedColorLine(int indentationLevel, string text, ConsoleColor? baseTextColor = null)
        {
            string prefix = string.Empty;
            foreach (var _ in Enumerable.Range(0, indentationLevel))
            {
                prefix = prefix + space + space;
            }
            prefix = prefix + end + ' ';

            WriteEmbeddedColorLine(prefix + text, baseTextColor);
        }
